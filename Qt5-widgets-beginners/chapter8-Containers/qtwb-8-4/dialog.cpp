...
void Dialog::init()
{
    ...
    auto firstElem = ui->tabWidget->children().at(1);

    for (auto elem : ui->tabWidget->children()) {
        qDebug() << " " << elem << " name " << elem;
    }

    qDebug() << " " << firstElem;
    // auto tabpanel2 = qobject_cast<QWidget *> (firstElem);
    //  tabpanel2->isVisible();


    // auto tabpanel3 =  qobject_cast<QWidget *> (ui->tabWidget->children().at(2));
    // tabpanel3->isTabVisible(false);
